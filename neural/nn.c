...
void network_save(NeuralNetwork *net, char *filename) {
    FILE *fp;
    char file_string[MAXCHAR];

    sprintf(file_string, "%s.net", filename);

    fp = fopen(file_string, "wb");
    fwrite(net, sizeof(NeuralNetwork), 1, fp);
    fclose(fp);

    mkdir(file_string);
}

NeuralNetwork* network_load(const char* file_string) {
    NeuralNetwork* net = (NeuralNetwork*)malloc(sizeof(NeuralNetwork));
    char entry[A];
    chdir(file_string);

    FILE* descriptor = fopen("descriptor", "r");
    fgets(entry, A, descriptor);
